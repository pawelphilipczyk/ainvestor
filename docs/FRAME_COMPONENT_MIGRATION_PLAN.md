...
- For **in-document navigation** that should drive **`<Frame>` reloads** (including named frames), Remix’s UI runtime exposes **`link`** (a mixin for `<a>` / `<area>`) and **`navigate()`** (imperative), both built on the **Navigation API** with frame-aware state. They replace ad hoc `location` / full reloads where a frame partial would suffice—but only if the destination behavior matches (correct `src` / partial HTML). See **`remix/ui`** exports and `docs/REMIX_V3_PACKAGES.md`.
...
- `docs/REMIX_V3_PACKAGES.md` — `remix/component` on the current alpha, `remix/ui` on beta, `<Frame>`, `run({ loadModule, resolveFrame })`.
- `remix/component` / `remix/ui` — **`link`**, **`navigate`**, **`NavigationOptions`** (frame-aware navigation alongside `<Frame>`; see installed runtime types in `node_modules` for the current surface).
