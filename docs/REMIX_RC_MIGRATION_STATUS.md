...
- **Stage:** 6 (behavior via primitives), close to done — `frame-submit.
  component.js`/`FrameSubmitEnhancement` is fully retired (see the newest row
  below). Stages 1–5, and the Stage 6 work through `6ddab0e` (advice's 3 forms
  ported to native `data-rmx-target`, PR #195), are merged on `main`.
- **Branch:** `claude/remix-rc2-migration-cwfgh6`, restarted fresh off `main`
  after PR #195 merged.
- **Green:** `npm run check`, `npm run typecheck`, `npm test` (594) and
  `npm run test:browser` (33) all pass.
...
`FrameSubmitEnhancement` is retired — there is nothing left of the form-mechanics
half of Stage 6 to port. What remains of Stage 6 is the navigation-primitive
items that were always separate from the frame-submit work:

**`tabs-nav` → `tabs/primitives`.** Expected to go the way of the sidebar
...
attempt so it is recorded with evidence rather than predicted — reason 3
only counts once actually tried against this codebase, per the Plan's
decision rule.
...
1. Stage 7 — `remix/ui/button` / `remix/ui/input`, then the dev tooling swaps.
