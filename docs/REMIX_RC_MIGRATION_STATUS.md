...
- **Stage:** 6 (behavior via primitives) is now **done**. `frame-submit.
  component.js`/`FrameSubmitEnhancement` is fully retired, and the one
  remaining item — tabs-nav → `tabs/primitives` — has been measured and
  recorded as not adopted (see the newest row below). Stages 1–5, and the
  Stage 6 work through `6ddab0e` (advice's 3 forms ported to native
  `data-rmx-target`, PR #195), are merged on `main`.
- **Branch:** `claude/remix-rc2-tabs-nav-port-d86b2p`, off `main`.
- **Green:** `npm run check`, `npm run typecheck`, `npm test` (594) and
  `npm run test:browser` (33) all pass — this branch is docs-only, no code
  changed.
...
Stage 6 is done — `FrameSubmitEnhancement` is retired, and tabs-nav has been
measured against `tabs/primitives` and kept hand-rolled (reason 3, see the
newest row above). Nothing else in Stage 6's inventory remains unattempted.
...
- **`tabs-nav` keeps its hand-rolled `<a>`-based tabs.** Measured live in
  Chromium against `remix/ui/tabs/primitives`, reason 3: `tab()` is a
  `<button>`-only mixin whose entire behavior is a client-side panel toggle
  with no `href`, no history, and no fetch, so it neither preserves a
  bookmarkable per-tab URL nor works without JavaScript — both properties
  `tabs-nav.tsx`'s real `<a>` navigation has today. See the newest row in
  *Done on this branch* above and `docs/REMIX_RC_MIGRATION_PLAN.md` Stage 6
  for the full trace.
